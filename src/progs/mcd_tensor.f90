program mcd_tensor
    ...
    type DebugType
        logical :: user_set
            !! User has requested some debugging features.
        logical :: print
            !! Enable debug printing.
        logical :: timer
            !! Add timer.
        logical :: add_ijaa
            !! add <ia||ja> terms in pfac for exc-exc integrals
    end type DebugType
    ...
    type Params
        ...
        character(len=:), allocatable :: outfile
            !! Optional output filename.
        integer :: f_state = -1
            !! Final state of interest, chosen automatically by default.
        integer :: max_state = -1
            !! Highest electronic state to consider in the summations.
            !! `-1` includes all possible states.
        logical :: use_gamma = .true.
            !! Use energy shift correction gamma.
        real(realwp) :: e_gamma = 1.0e-4_realwp
            !! Energy shift to avoid accidental degeneracies in summation.
        logical :: do_giao = .true.
            !! Include GIAO correction.
        logical :: do_guvcde = .false.
        ...
        logical :: show_timestamp = .true.
            !! Show timestamp in output.
        type(DebugType) :: debug = DebugType(.false., .false., .false., &
                                             .true.)
            !! Debug flags
    end type Params

    integer :: i, iab, istate, ispin_gs, ix, jstate, jx, qty_flag
    ...
    if (opts%do_guvcde) then
        if (opts%debug%timer) call write_time('GUVCDE write control')
        call write_control(iu_out, moldb, bsetdb, orbdb, err)
        call run%check(err)
    end if

    ! normalization coeffs
    if (opts%debug%timer) call write_time('Check AO normalization')
    call fix_norm_AOs(iu_out, moldb%n_at, orbdb%n_ao, moldb%at_crd, &
                      bsetdb, err, opts%debug%print)
    call run%check(err)

    call sec_header(2, 'Computation of one-electron integrals')
    if (opts%debug%timer) call write_time('1-e AO integral')
    ! overlap6
    qty_flag = 2**10 - 1
    ! qty_flag = ibset(qty_flag, 0)
    call overlap_ao_1e(iu_out, moldb%n_at, orbdb%n_ao, qty_flag, &
                       opts%do_guvcde, opts%in_mem, moldb%at_crd, &
                       bsetdb%nprim_per_at, bsetdb%info, ao_int, err)
    call run%check(err)
    ...
    if (opts%do_guvcde) then
        if (opts%debug%timer) call write_time('GUVCDE build MOs')
    ...
    if (opts%debug%timer) call write_time('AO to MO conversion')
    do iab = 1, orbdb%n_ab
        if (opts%debug%timer) call write_time('<i|r|j>')
        call convert_AO2MO(orbdb%n_ao, orbdb%n_mos(iab), &
                           orbdb%coef_mos(:,:,iab), ao_int%i_r_j, &
                           Smo_irj(:,:,:,iab), tmp_ao_arrN)
        if (opts%debug%timer) call write_time('<i|p|j>')
        call convert_AO2MO(orbdb%n_ao, orbdb%n_mos(iab), &
                           orbdb%coef_mos(:,:,iab), ao_int%i_p_j, &
                           Smo_ipj(:,:,:,iab), tmp_ao_arrN)
        if (opts%debug%timer) call write_time('<i|rxp|j>')
        call convert_AO2MO(orbdb%n_ao, orbdb%n_mos(iab), &
    ...
    if (opts%debug%timer) call write_time('Transition data')
    excdb = dfile%get_exc_data(get_dens=.true.)
    call run%check(dfile%error, &
    ...
    if (opts%debug%timer) call write_time('Build trans. amplitudes')
    ...
    if (opts%debug%timer) call write_time('Ground-state moments')
    p_gg_ab = f0
    r_gg_ab = f0
    rxp_gg_ab = f0
    ...
    if (opts%debug%timer) call write_time('Transition energies')
    allocate(ov_eieg(excdb%n_states), ov_eief(excdb%n_states))
    ef = excdb%g2e_energy(excdb%id_state)
    do istate = 1, excdb%n_states
        ei = excdb%g2e_energy(istate)
        if (opts%use_gamma) then
            ov_eieg(istate) = ei/(ei**2 + opts%e_gamma**2)
        else
        ...
        if (istate /= excdb%id_state) then
            de = ei - ef
            if (opts%use_gamma) then
                ov_eief(istate) = de/(de**2 + opts%e_gamma**2)
            else
    ...
    if (opts%do_giao) then
        if (opts%debug%timer) call write_time('GIAO terms')
            ...
            if (istate /= excdb%id_state) then
                ...
                pfac_r = &
                    sos_prefac_ejOei(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                                     ...
                                     opts%model_sos, opts%debug%add_ijaa)
                pfac_p = &
                    sos_prefac_ejOei(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                ...
                r_lk(:,istate,istate) = &
                    sos_ejOei(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                              t_mo(:,:,:,istate), pfac_r, model=opts%model_sos)
                p_lk(:,istate,istate) = &
                    sos_ejOei(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                              t_mo(:,:,:,istate), pfac_p, model=opts%model_sos)
                ...
                do jstate = 1, excdb%n_states
                    if (jstate /= istate) then
                        ...
                        if (opts%use_gamma) then
                            ov_eiej(istate,jstate) = &
                                de/(de**2 + opts%e_gamma**2)
                        else
    ...
    if (opts%debug%timer) call write_time('Prefactors')
    ...
    pfac_rxp = sos_prefac_ejOei(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                                ...
                                Smo_irxpj, opts%model_sos, opts%debug%add_ijaa)
    if (opts%do_giao) then
        r_lk(:,excdb%id_state,excdb%id_state) = &
            sos_ejOei(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                      t_mo(:,:,:,excdb%id_state), pfac_r, model=opts%model_sos)
        p_lk(:,excdb%id_state,excdb%id_state) = &
            sos_ejOei(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                      t_mo(:,:,:,excdb%id_state), pfac_p, model=opts%model_sos)
        r_lk(:,excdb%id_state,0) = &
            sos_eiOg(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                     t_mo(:,:,:,excdb%id_state), Smo_irj, model=opts%model_sos)
        p_lk(:,excdb%id_state,0) = &
            sos_eiOg(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                     t_mo(:,:,:,excdb%id_state), Smo_ipj, model=opts%model_sos)
        do istate = 1, excdb%n_states
            if (istate /= excdb%id_state) then
                forbid = .not.orbdb%openshell .and. &
    ...
    if (opts%debug%timer) call write_time('Transition moments')
    forbid = .not.orbdb%openshell .and. &
    ...
    if (opts%do_giao) then
        ...
        p_fg = -sos_eiOg(3, orbdb%n_ab, orbdb%n_mos, orbdb%n_els, &
                         ...
                         model=opts%model_sos) &
                / excdb%g2e_energy(excdb%id_state)
    endif
    ...
    G_if = f0
    if (opts%debug%print) write(iu_out, '(a)') 'NOW ON G_IF'
    do istate = 1, excdb%n_states
        if (istate /= excdb%id_state) then
            forbid = .not.orbdb%openshell .and. &
                excdb%ispin_exc(istate) /= ispin_gs
    ...
    if (opts%debug%timer) call write_time('LORG correction')
    if (opts%do_giao) then
        call sos_MCD_tensor_LORG_corr( &
...
contains
    ...
    subroutine parse_options(opts_db)
        ...
        parser = CmdLineArgsDB(progname='mcd_tensor')
        write(argval, &
              '("Failed to initialize the command-line parser of ",a)') &
            PROGTITLE
        call run%check(parser%error, argval)
        ...
        call parser%add_arg_real( &
            ...
            def_value=real(opts_db%e_gamma), min_value=0.0)
        ...
        write(argval, '("Unable to parse data for ",a)') PROGTITLE
        call run%check(parser%error, argval)

        ! Check debug flags
        if (parser%is_user_set('debug')) then
            opts_db%debug%user_set = .true.
            call parser%get_value('debug', svalues)
            do ival = 1, size(svalues)
                select case(locase(svalues(ival)))
                    case ('print')
                        opts_db%debug%print = .true.
                    case ('noprint')
                        opts_db%debug%print = .false.
                    case ('timer')
                        opts_db%debug%timer = .true.
                    case ('notimer')
                        opts_db%debug%timer = .false.
                    case ('ijaa')
                        opts_db%debug%add_ijaa = .true.
                    case ('noijaa')
                        opts_db%debug%add_ijaa = .false.
                end select
            end do
        end if
        ...
        if (parser%is_user_set('output')) then
            ...
            opts_db%outfile = trim(argval)
            open(newunit=iu_out, file=opts_db%outfile, action='write')
        end if
        call sec_header(-1, PROGTITLE)
        if (opts_db%debug%timer) call write_time('Option parser')
        ...
        if (parser%is_user_set('giao') &
            .and. parser%is_user_set('no-giao')) then
            call run%error%raise_error('opt', 'conflict', &
                'conflicting option for the definition of GIAO')
        end if

        if (parser%is_user_set('no-giao')) then
            opts_db%do_giao = .false.
        else if (parser%is_user_set('giao')) then
            opts_db%do_giao = .true.
        end if
        call write_param('GIAO correction', opts_db%do_giao)

        ! Check requirement for GUVCDE compatibility
        if (parser%is_user_set('guvcde')) opts_db%do_guvcde = .true.
        call write_param('Compatibility mode with GUVCDE', opts_db%do_guvcde)

        ! Check for value of e_gamma
        call parser%get_value('gamma', opts_db%e_gamma)
        opts_db%use_gamma = abs(opts_db%e_gamma) > tiny(opts_db%e_gamma)
        call write_param('Correction term against degeneracies', &
                         opts_db%use_gamma)
        if (opts_db%use_gamma) &
            call write_param('Value of the term (in Hartrees)', &
                             opts_db%e_gamma, .true.)

        ! Get input file name
        call parser%get_value('filename', argval)
        opts_db%infile = trim(argval)
        inquire(file=opts_db%infile, exist=exists)
        if (.not.exists) then
            write(argval, '("file ",a," does not exist.")') trim(opts_db%infile)
            call run%error%raise_error('file', 'not found', argval)
        end if
        call write_param('Input filename', opts_db%infile)

        ! Set default final state.
        if (parser%is_user_set('final')) then
            call parser%get_value('final', opts_db%f_state)
            call write_param('Final electronic state', opts_db%f_state)
        else
            call write_param('Final electronic state', 'automatic')
        end if

        if (parser%is_user_set('max-state')) then
            call parser%get_value('max-state', opts_db%max_state)
            call write_param('Highest excited state', opts_db%max_state)
        else
            call write_param('Highest excited state', 'include all')
        end if

        if (parser%is_user_set('exc-model')) then
            call parser%get_value('exc-model', argval)
            select case (trim(locase(argval)))
            ...
            case default
                call run%error%raise_error('opt', 'value', &
                    'unrecognized model for SOS excitation.')
            end select
        end if

        select case (opts_db%model_sos)
        case (1)
        ...
        case default
            call run%error%raise_deverror('case', &
                'list of supported SOS models needs updating')
        end select

        if (parser%is_user_set('no-timestamp')) opts_db%show_timestamp = .false.

        ! debugging mode - print information
        if (opts_db%debug%user_set) then
            call sec_header(2, 'Debugging parameters')
            call write_param("Extra printing", opts_db%debug%print)
            call write_param("Add timer", opts_db%debug%timer)
            call write_param("include <ia||ja> terms", opts_db%debug%add_ijaa)
        end if
