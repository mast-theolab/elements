...
function center_of_mass_mol(mol) result(com)
    !! Compute the center of mass.
    !!
    !! Computes the center of mass of a given structure.
    !!
    ...
    !$omp parallel do reduction(+:com,tot_mass)
    do ia = 1, mol%n_at
        com = com + mol%at_mas(ia)*mol%at_crd(:,ia)
        tot_mass = tot_mass + mol%at_mas(ia)
    end do
    !$omp end parallel do
    com = com / tot_mass
...
function inertia_moments_mol(mol) result(tensor)
    !! Compute the inertia moments tensor.
    !!
    !! Computes the inertia moments tensor.
    !!
...
end function inertia_moments_mol
...
subroutine superpose_arr(at_crd_new, at_crd_ref, weights, mask, rot_mat, &
    ...
    if (n_at /= size(at_crd_new, 2)) then
        ...
        call write_err('dev', &
            'Inconsistency between atomic coordinates', msg, &
            'superpose')
        stop 1
    end if
    ...
    if (present(weights)) then
        if (size(weights) /= n_at) then
            ...
            call write_err('dev', &
                'Inconsistency between weights and coordinates', msg, &
                'superpose')
            stop 1
        end if
        at_mass = weights
    ...
    if (present(mask)) then
        if (size(mask) /= n_at) then
            ...
            call write_err('dev', &
                'Inconsistency between mask and coordinates', msg, &
                'superpose')
            stop 1
        end if
        at_mask = mask
    ...
    if (info /= 0) then
        write(msg, '("xSyEV failed in row: ",i0)') info
        call write_err('std', &
            'Failure to diagonalize the quaternion matrix', msg)
        stop 1
    end if
    qevec = qmat
...
subroutine superpose_mol(mol, at_crd_ref, use_mass, weights, mask, &
                         rot_mat, trans_vec)
    !! Superpose a geometrical structure onto another.
    !!
    !! The routine tries to find the best overlap between a reference
    ...
    !! Molecule database.
    real(realwp), dimension(3,mol%n_at), intent(in) :: at_crd_ref
    !! Atomic coordinates (reference).
    logical, intent(in), optional :: use_mass
    !! Use atomic masses stored in `mol` as weights.
    real(realwp), dimension(mol%n_at), intent(in), optional :: weights
    !! Atomic weights (e.g., masses).
    logical, dimension(mol%n_at), intent(in), optional :: mask
    !! Mask, specifying which atom to include in superposition
    real(realwp), dimension(3,3), intent(out), optional :: rot_mat
    !! Rotation matrix from original to final orientation.
    real(realwp), dimension(3), intent(out), optional :: trans_vec
    ...
    real(realwp), dimension(mol%n_at) :: at_mass
    logical, dimension(mol%n_at) :: at_mask
    ...
    if (present(weights)) then
        if (present(use_mass)) then
            write(msg, '("Weight specifications given while requesting use &
                ...
                call write_err('dev', &
                    'Confusing request on weight specifications', msg, &
                    'superpose')
                stop 1
            end if
        at_mass = weights
    ...
    ! Main algorithm
    ! -- 1. get center of mass
    com_ref = center_of_mass_dim(mol%n_at, at_crd_ref, at_mass)
    com_new = center_of_mass_dim(mol%n_at, mol%at_crd, at_mass)
    ! -- 2. build the quaternion matrix
    qmat = f0
    do ia = 1, mol%n_at
        if (.not.at_mask(ia)) cycle
        ...
        yref = at_crd_ref(2,ia) - com_ref(2)
        zref = at_crd_ref(3,ia) - com_ref(3)
        xnew = mol%at_crd(1,ia) - com_new(1)
        ynew = mol%at_crd(2,ia) - com_new(2)
        znew = mol%at_crd(3,ia) - com_new(3)
    ...
    do ia = 1, mol%n_at
        ...
        ynew = mol%at_crd(2,ia) - com_new(2)
        znew = mol%at_crd(3,ia) - com_new(3)
        mol%at_crd(1,ia) = rmat(1,1)*xnew + rmat(2,1)*ynew + rmat(3,1)*znew
        mol%at_crd(2,ia) = rmat(1,2)*xnew + rmat(2,2)*ynew + rmat(3,2)*znew
        mol%at_crd(3,ia) = rmat(1,3)*xnew + rmat(2,3)*ynew + rmat(3,3)*znew
        mol%at_crd(:,ia) = mol%at_crd(:,ia) + com_ref
    end do

    if (present(rot_mat)) rot_mat = rmat
    if (present(trans_vec)) trans_vec = com_ref - com_new

end subroutine superpose_mol
