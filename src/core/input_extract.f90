...
function build_mol_data_fchk(dfile) result(mol)
    ...
    character(len=42), dimension(8) :: fchk_keys = [ &
        'Number of atoms                           ', &  ! 1
...
function build_bset_data_fchk(dfile) result(bset)
...
function build_orb_data_fchk(dfile) result(orb)
    !! Builds molecular specs. from Gaussian formatted checkpoint file.
    !!
    !! Parses a Gaussian formatted checkpoint file and extracts basic
    !! molecular specification data.
...
function build_exc_data_fchk(dfile) result(exc)
...
function build_vib_data_fchk(dfile, get_Lmat, get_Lmweig) result(vib)
